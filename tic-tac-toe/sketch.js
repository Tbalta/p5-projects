...
let w, h;
let ia = new Intelligence();
let iaP1 = new Intelligence();
let auto = true;
let P1auto = false;
...
function draw() {
	...
	if (auto && etat == etats.PLAYER2) {
		move = ia.getMove(grid);
		if (move) {
			grid[move[0]][move[1]] = 2;
			etat = etats.PLAYER1;
		} else{
			console.warn("no move available for p2");
			ia.noMove();
		}
	}

	if (P1auto && etat == etats.PLAYER1) {
		move = iaP1.getMove(grid);
		if (move) {
			grid[move[0]][move[1]] = 1;
			etat = etats.PLAYER2;
		} else{
			console.warn("no move available for p1");
			iaP1.noMove();
		}
	}
